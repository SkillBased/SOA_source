...
setup_mpack     = '%s ; import msgpack; src_bytes = msgpack.dumps(d)' % message
setup_xml       = '%s ; import dicttoxml, xmltodict; src = dicttoxml.dicttoxml(d)' % message
...
tests = [
    ...
    ('MessagePack', 'import msgpack; %s' % setup_mpack, 'msgpack.dumps(d)', 'msgpack.loads(src_bytes)'),
    ('xml', 'import dicttoxml, xmltodict; %s' % setup_xml, 'dicttoxml.dicttoxml(d)', 'xmltodict.parse(src)'),
